...
for i=[1:5]
    ...
    model_name = ['imagenet_finetune_car_',attr_type,'_',num2str(i),'_iter_',num2str(iters(i))];
    ...
    for layer = 1:length(layers)
        ...
        save_name = ['test_prob_car_',extract_layer_name,'_',num2str(i)];
        if exist(save_name,'dir')
            rmdir(save_name,'s');
        end

        minibatch_n = '10';%adjust to fit the real datasize
