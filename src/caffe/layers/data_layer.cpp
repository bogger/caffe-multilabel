...
void DataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK_GT(datum_height_, crop_size - 1);
  CHECK_GT(datum_width_, crop_size - 1);
  ...
  if (this->layer_param_.data_param().has_mean_file()) {
    ...
    //for (int i = 0; i < data_mean_.data_size(); i++) {
    LOG(INFO) << blob_proto.data(0);
    LOG(INFO) << blob_proto.data(1);
    LOG(INFO) << blob_proto.data(blob_proto.data_size() - 1);
    //}
  } else {
