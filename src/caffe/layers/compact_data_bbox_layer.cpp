...
void* CompactDataBBoxLayerPrefetch(void* layer_pointer) {
  CHECK(layer_pointer);
  CompactDataBBoxLayer<Dtype>* layer = static_cast<CompactDataBBoxLayer<Dtype>*>(layer_pointer);
  CHECK(layer);
  ...
  CHECK_EQ(crop_size, 256) << "crop size must be 256 currently";
  // datum scales
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    //std::cout << layer->iter_->key().ToString() << std::endl;
    string value = layer->iter_->value().ToString();
    char *buf = const_cast<char *>(value.data());
    int *pos = (int*)buf + 1;  // skip the first element which is 'label'
    CvMat mat = cvMat(1, 1000 * 1000, CV_8UC1, buf + 5 * sizeof(int));
    //LOG(INFO) << layer->iter_->key().ToString() << " " << *((int *)const_cast<char *>(value.data()));
    IplImage *img = cvDecodeImage(&mat, 1);
    int channels = img->nChannels;
    int width = img->width;
    int height = img->height;
    CHECK_EQ(width, 256) << "image width must be 256 currently";
    CHECK_EQ(height, 256) << "image height must be 256 currently";
    ...
    if (mirror && layer->PrefetchRand() % 2) {
      ...
      if (layer->output_labels_) {
        ...
        //std::cout << "Mirrored BBox: ";
        int tmp = pos[0];
        pos[0] = crop_size - pos[2];
        pos[2] = crop_size - tmp;
        for (int i = 0; i < 4; i++) {
          top_label[item_id * 4 + i] = (float)pos[i] / crop_size;
          //std::cout << top_label[item_id * 4 + i] << " ";
        }
        //std::cout << std::endl;
      }
    } else {
...
template <typename Dtype>
void CompactDataBBoxLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK_EQ(top->size(), 2) << "Data layer does not have two outputs.";
  if (top->size() == 1) {
  ...
  CvMat mat = cvMat(1, 1000 * 1000, CV_8UC1, const_cast<char *>(value.data()) + 5 * sizeof(int));
  IplImage *img = cvDecodeImage(&mat, 1);
...
template <typename Dtype>
void CompactDataBBoxLayer<Dtype>::CreatePrefetchThread() {
  ...
  // Create the thread.
  CHECK(!pthread_create(&(this->thread_), NULL, CompactDataBBoxLayerPrefetch<Dtype>,
