...
void* MultiViewDataLayerPrefetch(void* layer_pointer) {
  CHECK(layer_pointer);
  MultiViewDataLayer<Dtype>* layer = static_cast<MultiViewDataLayer<Dtype>*>(layer_pointer);
  CHECK(layer);
  Datum datum;
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (layer->output_labels_) {
      for (int v = 0; v < 10; v++)
      top_label[item_id * 10 + v] = datum.label();
...
template <typename Dtype>
void MultiViewDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  this->prefetch_data_.reset(new Blob<Dtype>(
        10 * this->layer_param_.data_param().batch_size(), datum.channels(),
        crop_size, crop_size));
...
template <typename Dtype>
void MultiViewDataLayer<Dtype>::CreatePrefetchThread() {
  ...
  // Create the thread.
  CHECK(!pthread_create(&(this->thread_), NULL, MultiViewDataLayerPrefetch<Dtype>,
        static_cast<void*>(this))) << "Pthread execution failed.";
}


INSTANTIATE_CLASS(MultiViewDataLayer);
