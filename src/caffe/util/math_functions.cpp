...
void caffe_gpu_gemm<float>(const CBLAS_TRANSPOSE TransA,
  ...
  /*CUBLAS_CHECK(cublasSgemm(Caffe::get_current_cublas_handle(),
	  cuTransB, cuTransA,
...
void caffe_gpu_geam<double>(const CBLAS_TRANSPOSE TransA,
  ...
  cublasOperation_t cuTransB =
      (TransB == CblasNoTrans) ? CUBLAS_OP_N : CUBLAS_OP_T;
  CUBLAS_CHECK(cublasDgeam(Caffe::get_current_cublas_handle(), cuTransB, cuTransA,
      N, M, &alpha, B, ldb, &beta,  A, lda, C, N));
}
...
void caffe_gpu_gemv<float>(const CBLAS_TRANSPOSE TransA, const int M,
  ...
  CUBLAS_CHECK(cublasSgemv(Caffe::get_current_cublas_handle(), cuTransA, N, M, &alpha,
      A, N, x, 1, &beta, y, 1));
}
...
void caffe_gpu_gemv<double>(const CBLAS_TRANSPOSE TransA, const int M,
  ...
  cublasOperation_t cuTransA =
      (TransA == CblasNoTrans) ? CUBLAS_OP_T : CUBLAS_OP_N;
  CUBLAS_CHECK(cublasDgemv(Caffe::get_current_cublas_handle(), cuTransA, N, M, &alpha,
      A, N, x, 1, &beta, y, 1));
}
...
void caffe_gpu_axpy<float>(const int N, const float alpha, const float* X,
    float* Y) {
  CUBLAS_CHECK(cublasSaxpy(Caffe::get_current_cublas_handle(), N, &alpha, X, 1, Y, 1));
}

template <>
void caffe_gpu_axpy<double>(const int N, const double alpha, const double* X,
    double* Y) {
  CUBLAS_CHECK(cublasDaxpy(Caffe::get_current_cublas_handle(), N, &alpha, X, 1, Y, 1));
}
