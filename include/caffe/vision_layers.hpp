...
class ConvolutionLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  static Blob<Dtype> bias_buffer_;
  static Blob<Dtype> trans_buffer_;

  shared_ptr<SyncedMemory> slave_col_buffer_;
  shared_ptr<SyncedMemory> slave_trans_buffer_;
  shared_ptr<SyncedMemory> slave_bias_buffer_;
  shared_ptr<SyncedMemory> slave_row_summer;
  shared_ptr<SyncedMemory> slave_bottom_buffer;
  shared_ptr<SyncedMemory> slave_top_buffer;
  shared_ptr<SyncedMemory> slave_weight_buffer_;

  shared_ptr<SyncedMemory> slave_weight_diff_buffer_;
  shared_ptr<SyncedMemory> slave_col_diff_buffer_;
  shared_ptr<SyncedMemory> master_weight_diff_buffer_;
  
  shared_ptr<SyncedMemory> bias_multiplier_;
  shared_ptr<SyncedMemory> row_sumer_;

  

  vector<cudaStream_t> layer_stream;
...
class LocallyConnectedLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  Blob<Dtype> col_buffer_; //holds columized bottom data and bottom diff
  Blob<Dtype> bias_buffer_;
  Blob<Dtype> trans_buffer_;  // data holds transformed top data in fp or top diff in bp, diff haven't been used
  
  shared_ptr<SyncedMemory> bias_multiplier_;

  shared_ptr<SyncedMemory> weight_entry_;			// holds entry to weight
  shared_ptr<SyncedMemory> col_entry_;				// holds entry to bottom data
  shared_ptr<SyncedMemory> bias_entry_;
  shared_ptr<SyncedMemory> result_entry_;			// holds entry to trans data
  shared_ptr<SyncedMemory> bias_multiplier_entry_;

  bool entry_initialized_;

  shared_ptr<SyncedMemory> col_diff_entry_;			// holds entry to bottom diff
  shared_ptr<SyncedMemory> trans_diff_entry_;		// holds entry to transformed weight diff
  shared_ptr<SyncedMemory> weight_diff_entry_;

  bool bp_entry_initialized_;
